...
pub fn run(command: &str, args: &[String], env_vars: &[String]) -> Result<()> {
    ...
    for arg in args {
        let arg_cstr = CString::new(arg.as_str()).context("Failed to convert argument to CString")?;
    ...
    for env_var in env_vars {
        let env_cstr = CString::new(env_var.as_str()).context("Failed to convert environment variable to CString")?;
...
pub fn get_library_dependencies(binary: &str) -> Result<Vec<String>> {
    ...
    if !output.status.success() {
        error!("ldd command failed: {}", String::from_utf8_lossy(&output.stderr));
